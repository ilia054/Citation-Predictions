...
class Generator(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.fc1 = nn.Linear(latent_dim, EMBEDDING_DIMENSION)
        self.fc2 = nn.Linear(EMBEDDING_DIMENSION, EMBEDDING_DIMENSION * 2)
        self.fc3 = nn.Linear(EMBEDDING_DIMENSION * 2, EMBEDDING_DIMENSION * 2)
        self.fc4 = nn.Linear(EMBEDDING_DIMENSION * 2, EMBEDDING_DIMENSION)
        self.dropout = nn.Dropout(0.2)
        self.batchnorm1 = nn.BatchNorm1d(EMBEDDING_DIMENSION * 2)
        self.batchnorm2 = nn.BatchNorm1d(EMBEDDING_DIMENSION * 2)

    def forward(self, z):
        x = F.relu(self.fc1(z))
        x = self.dropout(x)
        x = F.relu(self.batchnorm1(self.fc2(x)))
        x = F.relu(self.batchnorm2(self.fc3(x)))
        x = self.fc4(x)
        return x
...
def LF_NetLay(graph, feature_vectors):
  ...
  communities = graph.community_infomap(edge_weights='weight',trials=100)
...
def pre_train_G(graph, real_neighbor_pairs):
    ...
    optimizer_G = optim.RAdam(generator.parameters(), lr=G_LR)
    # Use MAE (L1 Loss) as the loss function
    # loss_function = nn.L1Loss()
    # Use MSE (L2 Loss) as the loss function
    loss_function = nn.MSELoss()
    early_stopping = EarlyStopping(patience=int(graph.number_of_nodes() ** 0.5), min_delta=0.01, save_embeddings=True)
    scheduler_G = ExponentialLR(optimizer_G, gamma=G_LEARNING_RATE_DECAY) 
    ...
    return early_stopping.best_embeddings if early_stopping.save_embeddings else None, generator, optimizer_G, loss_function,scheduler_G

def pre_train_D(real_neighbor_pairs, fake_embeddings):
    discriminator = Discriminator() 
    optimizer_D = optim.RAdam(discriminator.parameters(), lr=LR)
    loss_function = nn.BCELoss()
    early_stopping = EarlyStopping(patience=10, min_delta=0.01)  # Adjust patience as needed
    scheduler_D = ExponentialLR(optimizer_D, gamma=LEARNING_RATE_DECAY) 
    ...
    return discriminator, optimizer_D, loss_function,scheduler_D
 
def EmbedGAN(networkX_graph, node_embeddings, generator, discriminator, optimizer_G, optimizer_D, loss_G, loss_D,scheduler_G,scheduler_D, output_file):
...
def train_GAN(graph, walks, node_embeddings, generator, discriminator, optimizer_G, optimizer_D, loss_G, loss_D,scheduler_G,scheduler_D, num_epochs, batch_size, output_file):
...
def introduce_embedding_variations(embeddings, features, node_to_community_mapping, scale=0.35):
    """
...
def GAHNRL(g, feature_vectors, id_mapping, output_file, predictions_file):
    ...
    fake_embeddings,generator,optimizer_G, loss_G,scheduler_G = pre_train_G(Gn,real_neighbor_pairs)
    fake_embeddings_detached = fake_embeddings.detach()

    discriminator, optimizer_D, loss_D,scheduler_D = pre_train_D(real_neighbor_pairs, fake_embeddings_detached)
